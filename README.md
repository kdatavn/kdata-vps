### Lệnh Cài Đặt KLEMP (KDATA-VPS-LEMP) Trên Centos 6 & 7 & 8:
```
...
- [x] **`CentOS 7 x64` Khuyên dùng để cho hiệu suất tốt nhất. Mã nguồn KLEMP hiện tại đang phát triển chính trên CentOS 7 x64.**
- [ ] `CentOS 8 x64` MỚI và đang thử nghiệm. Tương lai gần KLEMP cũng sẽ phát triển song song cả trên CentOS 8 x64, hiện tại mới đang thử nghiệm.
- [ ] `CentOS 6 x64` KLEMP được phát triển trên phiên bản CentOS 6 x64, do KLEMP kế thừa mã nguồn của VPSSIM nên KLEMP cũng hoạt động tốt trên CentOS 6 x64, nên nếu có yêu cầu bắt buộc phải dùng CentOS 6 thì bạn có thể hoàn toàn yên tâm sử dụng, chỉ là về lâu dài thì các phiên bản cũ sẽ trờ nên lỗi thời, nên KLEMP chỉ phát triển từ CentOS 7 & 8.

----------------------------------------------

### Danh sách các phiên bản kết hợp đã qua quá trình cài đặt thử nghiệm thành công:
> Với `CentOS 6 x64` đã chạy thực nghiệm từ nhiều năm nay và cho kết quả ổn định, nên ở mục này chủ yếu thống kê cho phiên bản CentOS 7 & 8 và KLEMP cũng chỉ hỗ trợ phiên bản x64 chứ không hỗ trợ x32 như KLEMP bản gốc.
...
#### Ngoài việc sử dụng KLEMP làm VPS chạy website thông thường, giờ đây bạn cũng có thể sử dụng để làm VPS chạy Varnish Cache rất tiện dụng. Hiện tại mình đang chạy thành công trên Varnish 4.1, bản 6.xx mới hơn chút xíu nhưng mình chưa thử nghiệm ngon lành, nên khuyên dùng vẫn là Varnish 4.1
#### Mã thực thi: https://github.com/itvn9online/vpssim-free/tree/master/script/vpssim/menu/varnish
> Cách sử dụng: Trong KLEMP menu -> 25) Tien ich - Addons -> 23) Varnish Cache -> Chọn phiên bản Varnish mà bạn muốn cài đặt
...
> Cách sử dụng: Trong KLEMP menu -> 26) Update System -> 7) Thay phien phien ban OpenSSL
...
- Cài đặt nginx-1.18.0, đây là phiên bản ổn định và mới nhất của nginx tính đến thời điểm hiện tại, kết hợp với OpenSSL-1.1.1i thay cho bản openssl cũ của KLEMP, phiên bản này mới hỗ trợ đầy đủ HTTP/2.
	...
	- https://ftp.pcre.org/pub/pcre/ -> dùng bản 8.44 thay cho bản 8.39 của KLEMP.
	- https://www.zlib.net/ -> dùng bản 1.2.11 thay cho bản 1.2.8 của KLEMP.
...
> 2020/08/27
- Cho bạn lựa chọn MariaDB phiên bản 10.3, 10.2, 10.1, 10.0 và bản 5.5 thay vì bản 10.0 và 5.5 mặc định của KLEMP. Tự động config phù hợp với cấu hình server.
- Lựa chọn 6 phiên bản PHP : 7.2, 7.1, 7.0, 5.6, 5.5 hoặc 5.4 . KLEMP tự động config tối ưu PHP tùy theo cấu hình VPS và thay bạn có thể thay đổi PHP version thoải mái trong quá trình sử dụng.
